<?php
   ...
   {
      if(strcmp($check_str, "searchOnlineList"))
      {
         return SYMBOL_ERROR;
      }
      return $check_str;
   ...
   if(($searchTopicName = check_name($_GET["searchTopicName"])) == SYMBOL_ERROR)
   {
      sleep(DELAY_SEC);
      echo SYMBOL_ERROR;
      return;
   ...
   //----- query -----
   //***Step16 页面搜索SQl语句 起始
   $str_query1 = "select * from chatlogs where status=10";

 
   //TODO: trim space
   if (strlen($searchTopicName) > 0)
   {
      $str_query1 = $str_query1 . " AND (topic like '%$searchTopicName%') ";
   }
   ...
   if($result = mysqli_query($link, $str_query1)){      
      ...
      $return_string = $return_string . "<div class=\"toolMenu\">"
                                      ...
                                      . "<input type=\"hidden\" name=search_onlinelist_page_no value=1>"
                                      . "<input type=\"hidden\" name=search_onlinelist_page_size value=" . $page_size . ">";
      if ($page_num > 1)
      {
         for ($i = 0; $i < $page_num; $i++)
         {
            $return_string = $return_string . "<span class=\"search_onlinelist_page";
            if ($i + 1 == $page_default_no)
               $return_string = $return_string . " active";
            //***Step6 function name ==> clickSearchNewsPage
            $return_string = $return_string . "\" id=search_onlinelist_page_begin_no_" . ($i + 1) . " OnClick=clickSearchOnlineListPage(this," . ($i + 1) . ");>" . ($i + 1) . "</span>";
         }
      }
      //***Step7 function name ==> expandSearchNewsContentFunc
      $return_string = $return_string . "</span>"
                       . "<span class=\"btn NewsexpandSR\" OnClick=\"expandSearchOnlineListContentFunc();\">显示过长内容</span>"
                       . "</div>";                   
      ...
      else
      {
         ...
         while ($i < $row_number)
         {
            if ($page_count == 0)
            {
               $return_string = $return_string . "<div id=\"search_onlinelist_page" . $page_no . "\" ";
               if ($page_no == 1)
            ...
            if ($page_count < $page_size)
            {
               ...
               $chatLogsId = $row["id"];
               $chatLogsTopic = $row["topic"];
               $chatLogsUserName1 = $row["username1"];
               $chatLogsQq1 = $row["qq1"];
               $chatLogsCreatetime = $row["create_time"];
               $page_count_display = $page_count + 1;
               $return_string = $return_string 
                  . "<tr>"
                  . "<td>$page_count_display</td>"
                  . "<td><span class=\"topic fixWidth\">$chatLogsTopic</span></td>"
                  . "<td><span class=\"username1 fixWidth\">$chatLogsUserName1</span></td>"
                  . "<td><span class=\"qq1 fixWidth\">$chatLogsQq1</span></td>"
                  . "<td><span class=\"qq1 fixWidth\">$chatLogsId</span></td>"
                  . "<td><span class=\"chatLogsCreate_time fixWidth\">$chatLogsCreatetime</span></td>"
                  . "<td><A OnClick=\"chatSearchUsers($chatLogsId,'$chatLogsQq1','$chatLogsTopic');\">聊天</A><br/>"
                  . "</tr>";
      ...
      if ($page_num > 1)
      {
         for ($i = 0; $i < $page_num; $i++)
         {
            ...
            if ($i + 1 == $page_default_no)
               $return_string = $return_string . " active";
            $return_string = $return_string . "\" id=search_onlinelist_page_end_no_" . ($i + 1) . " OnClick=clickSearchOnlineListPage(this," . ($i + 1) . ");>" . ($i + 1) . "</span>";
         }
      }
      $return_string = $return_string . "</span>"
                                      . "</div>";
