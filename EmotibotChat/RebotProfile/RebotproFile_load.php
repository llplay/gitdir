<?php
   ...
   {
      if(strcmp($check_str, "searchRebotproFile"))
      {
         return SYMBOL_ERROR;
      }
      return $check_str;
   ...
   //----- query -----
   //***Step16 页面搜索SQl语句 起始
   $str_query1 = "select * from rebotprofile where status";
   ...
   if($result = mysqli_query($link, $str_query1)){      
      ...
      $return_string = $return_string . "<div class=\"toolMenu\">"
                                      ...
                                      . "<input type=\"hidden\" name=search_rebotprofile_page_no value=1>"
                                      . "<input type=\"hidden\" name=search_rebotprofile_page_size value=" . $page_size . ">";
      if ($page_num > 1)
      {
         for ($i = 0; $i < $page_num; $i++)
         {
            ...
            $return_string = $return_string . "\" id=search_rebotprofile_page_begin_no_" . ($i + 1) . " OnClick=clickSearchRebotproFilePage(this," . ($i + 1) . ");>" . ($i + 1) . "</span>";
         }
      }
      ...
      $return_string = $return_string . "</span>"
                       ...
                       . "<span class=\"btn NewsexpandSR\" OnClick=\"expandSearchRebotproFileContentFunc();\">显示过长内容</span>"
                       . "</div>";                   
      ...
      else
      {
         ...
         while ($i < $row_number)
         {
            if ($page_count == 0)
            {
               $return_string = $return_string . "<div id=\"search_rebotprofile_page" . $page_no . "\" ";
               if ($page_no == 1)
            ...
            if ($page_count < $page_size)
            {
               ...
               $return_string = $return_string 
                  ...
                  . "<td>$StatusStr</td>"
                  . "<td><A OnClick=\"actionSearchRebotproFile($rfId,$Status);\">$StatusAction</A><br/>"
                  . "<A OnClick=\"modifySearchRebotproFile($rfId);\">修改</A><br/>"
                  . "</tr>";
      ...
      if ($page_num > 1)
      {
         for ($i = 0; $i < $page_num; $i++)
         {
            ...
            if ($i + 1 == $page_default_no)
               $return_string = $return_string . " active";
            $return_string = $return_string . "\" id=search_rebotprofile_page_end_no_" . ($i + 1) . " OnClick=clickSearchRebotproFilePage(this," . ($i + 1) . ");>" . ($i + 1) . "</span>";
         }
      }
      $return_string = $return_string . "</span>"
                                      . "</div>";
