<?php
   ...
   {
      if(strcmp($check_str, "searchRobotproFile"))
      {
         return SYMBOL_ERROR;
      }
      return $check_str;
   ...
   {
      //----- check str length -----
      if(strlen($check_str, "utf8") > STR_LENGTH)
      {
         return SYMBOL_ERROR;
      }       
      //----- replace "<" to "&lt" -----
   ...
   if(($searchRFQA = check_name($_GET["searchRFQA"])) == SYMBOL_ERROR)
   {
      sleep(DELAY_SEC);
      echo SYMBOL_ERROR;
      return;
   ...
   //----- query -----
   //***Step16 页面搜索SQl语句 起始
   $str_query1 = "select * from robotprofile where status";
   ...
   //TODO: trim space
   if (strlen($searchRFQA) > 0)
   {
      $str_query1 = $str_query1 . "AND (strQ like '%$searchRFQA%' OR strA like '%$searchRFQA%') ";
   }
   ...
   if($result = mysqli_query($link, $str_query1)){      
      ...
      $return_string = $return_string . "<div class=\"toolMenu\">"
                                      ...
                                      . "<input type=\"hidden\" name=search_robotprofile_page_no value=1>"
                                      . "<input type=\"hidden\" name=search_robotprofile_page_size value=" . $page_size . ">";
      if ($page_num > 1)
      {
         for ($i = 0; $i < $page_num; $i++)
         {
            $return_string = $return_string . "<span class=\"search_robotprofile_page";
            if ($i + 1 == $page_default_no)
               $return_string = $return_string . " active";
            //***Step6 function name ==> clickSearchNewsPage
            $return_string = $return_string . "\" id=search_robotprofile_page_begin_no_" . ($i + 1) . " OnClick=clickSearchRobotproFilePage(this," . ($i + 1) . ");>" . ($i + 1) . "</span>";
         }
      }
      ...
      $return_string = $return_string . "</span>"
                       ...
                       . "<span class=\"btn NewsexpandSR\" OnClick=\"expandSearchRobotproFileContentFunc();\">显示过长内容</span>"
                       . "</div>";                   
      ...
      else
      {
         ...
         while ($i < $row_number)
         {
            if ($page_count == 0)
            {
               $return_string = $return_string . "<div id=\"search_robotprofile_page" . $page_no . "\" ";
               if ($page_no == 1)
            ...
            if ($page_count < $page_size)
            {
               ...
               $return_string = $return_string 
                  ...
                  . "<td><span class=\"strQ fixWidth\">$strQ</span></td>"
                  . "<td><span class=\"strA fixWidth\">$strA</span></td>"
                  . "<td>$StatusStr</td>"
                  . "<td><A OnClick=\"actionSearchRobotproFile($rfId,$Status);\">$StatusAction</A><br/>"
                  . "<A OnClick=\"modifySearchRobotproFile($rfId);\">修改</A><br/>"
                  . "</tr>";
      ...
      if ($page_num > 1)
      {
         for ($i = 0; $i < $page_num; $i++)
         {
            ...
            if ($i + 1 == $page_default_no)
               $return_string = $return_string . " active";
            $return_string = $return_string . "\" id=search_robotprofile_page_end_no_" . ($i + 1) . " OnClick=clickSearchRobotproFilePage(this," . ($i + 1) . ");>" . ($i + 1) . "</span>";
         }
      }
      $return_string = $return_string . "</span>"
                                      . "</div>";
