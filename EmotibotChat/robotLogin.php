...
//   echo "$login_name";echo "---";return;
   ...
   {
     sleep(DELAY_SEC);
     header("Location:robotMain.php?cmd=err");
     exit();
   }
   ...
   //echo "$login_name";
   //echo "$qqnumber"; 
   $str_query_user = "select username, status  from users where status in (2,3,6,7) and username = '$login_name'";
   ...
   {   //query success
      ...
      if ($rownum_qq > 0 && $rownum_user > 0)
      {
         ...
         session_write_close();
         header("Location:robotIndex.php");
         exit();
      }
      else 
      {
