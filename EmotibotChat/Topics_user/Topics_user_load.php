<?php
   ...
   {
      if(strcmp($check_str, "searchTopics_user"))
      {
         return SYMBOL_ERROR;
      }
      return $check_str;
   ...
   if(($searchTopics_name = check_name($_GET["searchTopics_name"])) == SYMBOL_ERROR)
   {
      sleep(DELAY_SEC);
      echo SYMBOL_ERROR;
      return;
   ...
   //----- query -----
   //***Step16 页面搜索SQl语句 起始
   $str_query1 = "select * from topics where status = 1";

   
   //TODO: trim space
   if (strlen($searchTopics_name) > 0)
   {
      $str_query1 = $str_query1 . " AND (name like '%$searchTopics_name%') ";
   }
   ...
   if($result = mysqli_query($link, $str_query1)){      
      ...
      $return_string = $return_string . "<div class=\"toolMenu\">"
                                      ...
                                      . "<input type=\"hidden\" name=search_topics_user_page_no value=1>"
                                      . "<input type=\"hidden\" name=search_topics_user_page_size value=" . $page_size . ">";
      if ($page_num > 1)
      {
         for ($i = 0; $i < $page_num; $i++)
         {
            $return_string = $return_string . "<span class=\"search_topics_user_page";
            if ($i + 1 == $page_default_no)
               $return_string = $return_string . " active";
            //***Step6 function name ==> clickSearchNewsPage
            $return_string = $return_string . "\" id=search_topics_user_page_begin_no_" . ($i + 1) . " OnClick=clickSearchTopics_userPage(this," . ($i + 1) . ");>" . ($i + 1) . "</span>";
         }
      }
      //***Step7 function name ==> expandSearchNewsContentFunc
      $return_string = $return_string . "</span>"
                       . "<span class=\"btn NewsexpandSR\" OnClick=\"expandSearchTopics_userContentFunc();\">显示过长内容</span>"
                       . "</div>";                   
      ...
      else
      {
         ...
         while ($i < $row_number)
         {
            if ($page_count == 0)
            {
               $return_string = $return_string . "<div id=\"search_topics_user_page" . $page_no . "\" ";
               if ($page_no == 1)
            ...
            if ($page_count < $page_size)
            {
               ...
               $return_string = $return_string 
                  ...
                  . "<td><span class=\"topics fixWidth\">$topicName</span></td>"
                  . "<td><A OnClick=\"modifySearchTopics_user($topicId, '$topicName');\">选择</A><br/></td>"
                  . "</tr>";
      ...
      if ($page_num > 1)
      {
         for ($i = 0; $i < $page_num; $i++)
         {
            ...
            if ($i + 1 == $page_default_no)
               $return_string = $return_string . " active";
            $return_string = $return_string . "\" id=search_topics_user_page_end_no_" . ($i + 1) . " OnClick=clickSearchTopics_userPage(this," . ($i + 1) . ");>" . ($i + 1) . "</span>";
         }
      }
      $return_string = $return_string . "</span>"
                                      . "</div>";
