<?php
   ...
   {
      if(strcmp($check_str, "searchUsersName"))
      {
         return SYMBOL_ERROR;
      }
      return $check_str;
   ...
   if(($searchUsersName = check_name($_GET["searchUsersName"])) == SYMBOL_ERROR)
   {
      sleep(DELAY_SEC);
      echo SYMBOL_ERROR;
      return;
  ...
  // }
   ...
   //----- query -----
   //***Step16 页面搜索SQl语句 起始
   $str_query1 = "select * from users ";// status AND ";
...
//   }
   ...
   //TODO: trim space
   if (strlen($searchUsersName) > 0)
   {
      $str_query1 = $str_query1 . "  where username like '%$searchUsersName%' ";
   }
   ...
   if($result = mysqli_query($link, $str_query1)){      
      ...
      $return_string = $return_string . "<div class=\"toolMenu\">"
                                      ...
                                      . "<input type=\"hidden\" name=search_users_page_no value=1>"
                                      . "<input type=\"hidden\" name=search_users_page_size value=" . $page_size . ">";
      if ($page_num > 1)
      {
         for ($i = 0; $i < $page_num; $i++)
         {
            $return_string = $return_string . "<span class=\"search_users_page";
            if ($i + 1 == $page_default_no)
               $return_string = $return_string . " active";
            //***Step6 function name ==> clickSearchNewsPage
            $return_string = $return_string . "\" id=search_users_page_begin_no_" . ($i + 1) . " OnClick=clickSearchUsersPage(this," . ($i + 1) . ");>" . ($i + 1) . "</span>";
         }
      }
      ...
      $return_string = $return_string . "</span>"
                       ...
                       . "<span class=\"btn NewsexpandSR\" OnClick=\"expandSearchUsersContentFunc();\">显示过长内容</span>"
                       . "</div>";                   
      ...
      else
      {
         ...
         while ($i < $row_number)
         {
            if ($page_count == 0)
            {
               $return_string = $return_string . "<div id=\"search_users_page" . $page_no . "\" ";
               if ($page_no == 1)
            ...
            if ($page_count < $page_size)
            {
               ...
               $userId = $row["id"];
               $userPhone = $row["phone"];
               $userWeChat = $row["wechatid"];
               $Status = $row["status"];
               $StatusStr = "";
               if (($Status & 1) > 0)
                  $StatusStr = $StatusStr . "用户 ";
               if (($Status & 2) > 0)
                  $StatusStr = $StatusStr . "机器人 ";
               if (($Status & 4) > 0)
                  $StatusStr = $StatusStr . "管理者 ";
               if ($StatusStr == "")
                  $StatusStr = "无权限";
               ...
               $return_string = $return_string 
                  ...
                  . "<td><span class=\"userId fixWidth\">$userId</span></td>"
                  . "<td><span class=\"userPhone fixWidth\">$userPhone</span></td>"
                  . "<td><span class=\"userWeChat fixWidth\">$userWeChat</span></td>"
                  . "<td>$StatusStr</td>"
                  . "<td><A OnClick=\"modifySearchUsers('$userId');\">修改</A><br/>"
                  . "</tr>";
      ...
      if ($page_num > 1)
      {
         for ($i = 0; $i < $page_num; $i++)
         {
            ...
            if ($i + 1 == $page_default_no)
               $return_string = $return_string . " active";
            $return_string = $return_string . "\" id=search_users_page_end_no_" . ($i + 1) . " OnClick=clickSearchUsersPage(this," . ($i + 1) . ");>" . ($i + 1) . "</span>";
         }
      }
      $return_string = $return_string . "</span>"
                                      . "</div>";
