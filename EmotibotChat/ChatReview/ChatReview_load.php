<?php
   ...
   {
      if(strcmp($check_str, "searchChatReview"))
      {
         return SYMBOL_ERROR;
      }
      return $check_str;
   ...
   if(($searchReviewUsername = check_name($_GET["searchReviewUsername"])) == SYMBOL_ERROR)
   {
      sleep(DELAY_SEC);
      echo SYMBOL_ERROR;
      return;
   }
   $searchReviewfrom = $_GET["searchReviewfrom"];
   $searchReviewto = $_GET["searchReviewto"];
   $searchChatLogsStatus = $_GET["searchChatLogsStatus"];
   ...
   if($result = mysqli_query($link, $str_query1)){      
      ...
      $return_string = $return_string . "<div class=\"toolMenu\">"
                                      ...
                                      . "<input type=\"hidden\" name=search_chatreview_page_no value=1>"
                                      . "<input type=\"hidden\" name=search_chatreview_page_size value=" . $page_size . ">";
      if ($page_num > 1)
      {
         for ($i = 0; $i < $page_num; $i++)
         {
            ...
            $return_string = $return_string . "\" id=search_chatreview_page_begin_no_" . ($i + 1) . " OnClick=clickSearchChatReviewPage(this," . ($i + 1) . ");>" . ($i + 1) . "</span>";
         }
      }
      ...
      else
      {
         ...
         while ($i < $row_number)
         {
            if ($page_count == 0)
            {
               $return_string = $return_string . "<div id=\"search_chatreview_page" . $page_no . "\" ";
               if ($page_no == 1)
            ...
            if ($page_count < $page_size)
            {
               ...
               $return_string = $return_string 
                  ...
                  . "<td>$ChatId</td>"
                  . "<td><span class=\"username1 fixWidth\">$username1</span></td>"
                  . "<td><span class=\"username2 fixWidth\">$username2</span></td>"
                  . "<td><span class=\"topic fixWidth\">$topic</span></td>"
                  . "<td>$counter</td>"
                  . "<td>$upload_time</td>"
                  . "<td>$StatusStr</br>"
                  . "$reviewer</td>"
