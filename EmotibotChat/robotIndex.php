<?php
   ...
   if ($_SESSION["QQ"] == "" || $_SESSION["username"] == "")
   {
      session_write_close();
      sleep(DELAY_SEC);
      header("Location:robotMain.php?cmd=main");
      exit();
   }
   ...
   $str_query1 = "Select F.FunctionName as func_name, F.FunctionType as func_type, 
      ...
      order by F.FunctionType, F.Rank";
   $func_array = Array();
   ...
   if ($result = mysqli_query($link, $str_query1))
   {
      while ($row = mysqli_fetch_assoc($result))
      {
         ...
         if ($row["func_type"] == 1)
            array_push($ordinary_array,$sf);
         if ($row["func_type"] == 2)
            array_push($iv_array,$sf);
         if ($row["func_type"] == 3)
            array_push($app_array,$sf);
         if ($row["func_type"] == 4)
            array_push($manage_array,$sf);
...
<?php
   echo "<li id=0 class='active'><a href='javascript:void(0)'><span class='tabIcon QQ'></span><span class='nav-label'>在线列表</span></a></li>";
   echo "<li id=1><a href='javascript:void(0)'><span class='tabIcon QQ'></span><span class='nav-label'>存储对话</span></a></li>";
?>
                     </ul>
                  </li>
               </ul>
...
<?php
   ...
   include("OnlineList/OnlineList_list.php");
   echo "</div>";

   echo "<div class='container2 searchNewsC' style='display:none;'>";
   include("LoadDialog/LoadDialog_list.php");
   echo "</div>";
