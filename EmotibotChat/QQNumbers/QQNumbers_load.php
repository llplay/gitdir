<?php
   ...
   {
      if(strcmp($check_str, "searchQQNumbers"))
      {
         return SYMBOL_ERROR;
      }
      return $check_str;
   ...
   $searchQQ = $_GET["searchQQ"];
   if (strlen($searchQQ) > 0 && check_number($searchQQ) == SYMBOL_ERROR)
   {
      sleep(DELAY_SEC);
      echo SYMBOL_ERROR;
      return;
   ...
   //----- query -----
   //***Step16 页面搜索SQl语句 起始
   $str_query1 = "select * from qqnumbers where status";
   ...
   //TODO: trim space
   if (strlen($searchQQ) > 0)
   {
      $str_query1 = $str_query1 . "AND (qq like '%$searchQQ%') ";
   }
   ...
   if($result = mysqli_query($link, $str_query1)){      
      ...
      $return_string = $return_string . "<div class=\"toolMenu\">"
                                      ...
                                      . "<input type=\"hidden\" name=search_qqnumbers_page_no value=1>"
                                      . "<input type=\"hidden\" name=search_qqnumbers_page_size value=" . $page_size . ">";
      if ($page_num > 1)
      {
         for ($i = 0; $i < $page_num; $i++)
         {
            $return_string = $return_string . "<span class=\"search_qqnumbers_page";
            if ($i + 1 == $page_default_no)
               $return_string = $return_string . " active";
            //***Step6 function name ==> clickSearchNewsPage
            $return_string = $return_string . "\" id=search_qqnumbers_page_begin_no_" . ($i + 1) . " OnClick=clickSearchQQNumbersPage(this," . ($i + 1) . ");>" . ($i + 1) . "</span>";
         }
      }
      ...
      $return_string = $return_string . "</span>"
                       ...
                       . "<span class=\"btn NewsexpandSR\" OnClick=\"expandSearchQQNumbersContentFunc();\">显示过长内容</span>"
                       . "</div>";                   
      ...
      else
      {
         ...
         while ($i < $row_number)
         {
            if ($page_count == 0)
            {
               $return_string = $return_string . "<div id=\"search_qqnumbers_page" . $page_no . "\" ";
               if ($page_no == 1)
            ...
            if ($page_count < $page_size)
            {
               ...
               $return_string = $return_string 
                  ...
                  . "<td><span class=\"strQQ fixWidth\">$qq</span></td>"
                  . "<td><span class=\"strOwner fixWidth\">$ownerStr</span></td>"
                  . "<td>$StatusStr</td>"
                  . "<td><A OnClick=\"actionSearchQQNumbers($qq,$Status);\">$StatusAction</A><br/>"
                  //. "<A OnClick=\"modifySearchQQNumbers($qq,$owner);\">修改</A><br/>"
                  . "</tr>";
      ...
      if ($page_num > 1)
      {
         for ($i = 0; $i < $page_num; $i++)
         {
            ...
            if ($i + 1 == $page_default_no)
               $return_string = $return_string . " active";
            $return_string = $return_string . "\" id=search_qqnumbers_page_end_no_" . ($i + 1) . " OnClick=clickSearchQQNumbersPage(this," . ($i + 1) . ");>" . ($i + 1) . "</span>";
         }
      }
      $return_string = $return_string . "</span>"
                                      . "</div>";
